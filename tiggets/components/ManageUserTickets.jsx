...
export default function ManageUserTickets({ targetUser, tickets, role }) {
  ...
  // Filter logic for this specific user's tickets
  ...
  return (
    <div className="w-full font-text text-foreground">
      ...
      <Link href="/user-management" className="inline-flex items-center gap-2 text-zinc-500 hover:text-[#3b5949] transition-colors mb-6 font-medium">
      ...
      <div className="bg-[#e2e2e2] pt-8 flex flex-col min-h-[600px] rounded-t-md shadow-sm border border-zinc-300">
        ...
        {showFilters && (
          <div className="px-6 mb-8 grid grid-cols-1 sm:grid-cols-3 gap-4 animate-in fade-in slide-in-from-top-2">
            <select name="type" value={filters.type} onChange={handleFilterChange} className="bg-white border border-zinc-300 rounded-sm px-3 py-2 text-xs outline-none focus:border-[#3b5949] text-zinc-500 cursor-pointer">
            ...
            <select name="status" value={filters.status} onChange={handleFilterChange} className="bg-white border border-zinc-300 rounded-sm px-3 py-2 text-xs outline-none focus:border-[#3b5949] text-zinc-500 cursor-pointer">
        ...
        {/* Reusing your exact TicketList component! */}
